...
//-----------------------------------------------------------------------------
//(yh) copied from above

inline void DisplayModel2(model& u) {
	//printf("draw.h - display model");
	auto M = u;
	...
	for (int i = 0; i < M.t_size; i++) {
		...
		Point3d cluster_color = Point3d(0, 0, 255);
...
inline void DisplayModelWireFrame2(model& u) {
//auto M = u.getModel();
	auto M = u;
...
void drawAll2() {
	...
	DisplayModel2(*finalobjmodel);
	DisplayModelWireFrame2(*finalobjmodel);
